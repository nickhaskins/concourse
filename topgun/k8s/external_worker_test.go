...
var _ = Describe("team external workers through separate deployments", func() {
	...
	AfterEach(func() {
		helmDestroy(releaseName + "-web")
		helmDestroy(releaseName + "-worker")

		Wait(Start(nil, "kubectl", "delete", "namespace", namespace+"-web", "--wait=false"))
		Wait(Start(nil, "kubectl", "delete", "namespace", namespace+"-worker", "--wait=false"))
